    ...
    with open(filename, 'rb') as file:
        ...
        while remaining_header > 0 and opt_type != 0:
            ...
            if opt_type == option_type:
                ...
                if json_only and is_json(comment) or not json_only:
...
def extract_comment(filename, option_type=1, json_only=True):
    ...
    return {} if json_only else ""


def is_pcapng(file):
    ret = subprocess.run(f'file {file}', shell=True, capture_output=True)

    # TODO: Make sure this isn't macOS specific
    return b': pcap-ng capture file ' in ret.stdout
...
def is_pcap(file):
    ...
    return b': tcpdump capture file' in ret.stdout
...
def insert_comment(filename_in, comment, filename_out=None):
    ...
    if not is_pcapng(filename_in):
        ...
        subprocess.call(
            f'tshark -F pcapng -r {re.escape(filename_in)} -w {re.escape(fname_in)}',
    ...
    with open(fname_in, 'rb') as file_in:
        if filename_out is None:
            ...
            filename_out = f'{filename}_commented{file_ext}'
        ...
        if comment_length_old == 0:
            ...
            opt_padding = 4 - opt_length % 4 if opt_length % 4 != 0 else 0
            remaining_header -= 4
        ...
        while data := file_in.read(1024 * 1024):
            file_out.write(data)
    file_out.close()
